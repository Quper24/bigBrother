...
const API_URI = 'https://mountain-oval-juice.glitch.me/';
...
file.addEventListener('change', async () => {
  if (file.files.length > 0) {
    const base64 = await modules_toBase64(file.files[0]);
...
const controlField = (btn, list, offList) => {
  ...
  list.addEventListener('click', _ref => {
    let {
      target
    } = _ref;
